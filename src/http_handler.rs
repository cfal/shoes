...
impl TcpServerHandler for HttpTcpServerHandler {
    async fn setup_server_stream(
        ...
        let (remote_location, initial_remote_data) = if line.starts_with("CONNECT ") {
            ...
            let remote_location = Location::new(Address::from(&domain_name)?, port);
            ...
            (remote_location, initial_remote_data)
        } else {
            ...
            let remote_location = match address.find(':') {
                Some(i) => {
                    ...
                    let port = address[i + 1..]
                        .parse::<u16>()
                        .map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e))?;
                    Location::new(Address::from(&address[0..i])?, port)
                }
                None => Location::new(Address::from(address)?, 80),
            };
