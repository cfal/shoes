...
///
/// `ZlibDecoder::read_to_end` is NOT usable here, which was measured rather
/// than assumed: on a truncated stream it reports success and returns a short
/// payload, at every level of truncation, even when only the four-byte Adler
/// trailer is missing. A short payload would leave a domain trie with a clipped
/// label array -- a matcher that is quietly wrong rather than one that fails.
/// Driving `Decompress` to `StreamEnd` makes the trailer, and with it the
/// checksum, mandatory.
